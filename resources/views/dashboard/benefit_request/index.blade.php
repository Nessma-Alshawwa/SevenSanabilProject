                        ...
                        <tbody>
                            @foreach ($BenefitRequests as $BenefitRequest)
                                <tr>
                                    ...
                                    <td><span class="badge {{ $BenefitRequest->status == 0 ? 'badge-danger' : ( $BenefitRequest->status == 1 ? 'badge-success' : ( $BenefitRequest->status == 2 ? 'badge-warning' : ( $BenefitRequest->status == 3 ? 'badge-info' : ( $BenefitRequest->status == 4 ? 'badge-secondary' : '' ) ) ) ) }}">{{ config('constance.status.' .$BenefitRequest->status) }}</span></td>
                                                            ...
                                                            <div class="modal-body">
                                                                <form action="{{URL('/dashboard/benefit_request/edit_status/' . $BenefitRequest->id) }}" method="POST" id="BenefitRequestStatus-form">
                                                                    ...
                                                                    <select class="form-control custom-select" name="status">
                                                                        ...
                                                                        @foreach (config('constance.status') as $key => $value)
